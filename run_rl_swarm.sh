...
while true; do
    ...
    case $ab in
        ...
        *)      
            if [ -n "$SELECTED_SWARM" ]; then
                echo ">>> Invalid SELECTED_SWARM value: $SELECTED_SWARM. Please set A or B."
                exit 1
            else
...
while true; do
    ...
    case $pc in
        ...
        *) 
            if [ -n "$PARAM_B_CHOICE" ]; then
                echo ">>> Invalid PARAM_B_CHOICE value: $PARAM_B_CHOICE. Please set one of [0.5, 1.5, 7, 32, 72]."
                exit 1
            else
